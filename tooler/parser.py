...
def _match_param_type(fn, param, value):
  ...
  if isinstance(param.default, int) or param.annotation == int:
    # `int` is automatically converted from the stdin
    return int(value)
...
class DefaultParser(Parser):
  def __init__(self, shorthands=None):
    ...
    for key in self.shorthands.keys():
      assert len(key) == 1 and key in ascii_letters, "Shorthand keys must be single letters"
  ...
  def usage(self, fn):
    ...
    return (
      "Usage:\n" +
      "\n".join(
        _key_usage(key_string, param)
        for key_string, param in key_strings.items()
      )
    )
  ...
  def _parse(self, fn, doc, selector, args):
    ...
    while idx < len(args):
      if args[idx] in ("-", "--") or not args[idx].startswith("-"):
        if len(keyword):
          raise CommandParseException("Positional arguments not valid after a keyword")
        ...
        if key in keyword:
          raise CommandParseException("Saw multiple values for an argument: " + key)

        # Add support for --flag, and --no-flag
        if key in boolean:
          if value is not None:
            raise CommandParseException("Value provided to boolean key: %s" % key)
          boolean[key] = True
          continue
        ...
        if value is None:
          ...
          if idx >= len(args):
            raise CommandParseException("No value provided for argument: %s" % key)
          value = args[idx]
          idx += 1
    ...
    for key, param in signature.parameters.items():
      if key in boolean:
        ...
        if param.annotation and param.annotation != inspect.Parameter.empty:  # noqa: W503
          ...
          positional = [
              _match_annotation_type(fn, single_annotation, value) for value in positional
          ]
        ...
        if key in keyword:
          ...
          raise CommandParseException("No value provided for required argument: " + key)
