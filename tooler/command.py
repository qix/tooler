...
def _value(fn, param, value):
    """
    Convert a value passed to a tooler function to the correct type, given a default or an annotation
    """
...
def default_parser(fn, args):
    ...
    boolean = {
        ...
        if param.default in (True, False) or param.annotation == bool
    }
        ...
        if not args[idx].startswith("-"):
            ...
            if key in boolean:
                if value is not None:
                    raise CommandParseException(
                        f"Expected boolean value for parameter: {key}"
                    )
                value = True
    ...
    for key, param in signature.parameters.items():
        ...
        if key in keyword:
            ...
            # argument
            args.append(_value(fn, param, positional.pop(0)))
